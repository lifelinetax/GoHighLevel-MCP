 ...
 * HTTP version for Claude/ChatGPT web integration
 * Updated: CORS fixed for Claude.ai, connection test fixed, /mcp route added
 */
...
class GHLMCPHttpServer {
  ...
  private setupExpress(): void {
    // FIX 1: Added claude.ai to allowed origins
    this.app.use(cors({
  ...
  private setupRoutes(): void {
    ...
    // FIX 3: SSE and MCP endpoints — both route to the same handler for Claude/ChatGPT compatibility
    ...
    this.app.get('/', (req, res) => {
      res.json({
        ...
        tools: this.getToolsCount(),
        documentation: 'https://github.com/mastanley13/GoHighLevel-MCP'
      });
    });
  }
  ...
  /**
   * FIX 2: Test GHL API connection using environment variables directly
   * Replaces the old method that used a hardcoded location ID
   */
  private async testGHLConnection(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`GHL API connection test failed: HTTP ${response.status}`);
      }
      console.log('[GHL MCP HTTP] ✅ GHL API connection successful');
      console.log(`[GHL MCP HTTP] Connected to location: ${locationId}`);
    } catch (error) {
